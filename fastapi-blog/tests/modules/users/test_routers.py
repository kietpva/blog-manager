...
def test_admin_partial_update_calls_service_and_returns_admin_user():
    ...
    This test verifies that:
      - The endpoint returns a 200 status code.
      - The response body contains the admin user's data with the correct id and role.
    ...
    service = Mock()
    user_id = uuid4()
    service.admin_partial_update.return_value = _admin_user_read_obj(user_id)
    client, me = _build_client(service)
    ...
    service.admin_partial_update.assert_called_once()
    kwargs = service.admin_partial_update.call_args.kwargs
    assert kwargs["user_id"] == user_id
    assert kwargs["current_user"] is me
